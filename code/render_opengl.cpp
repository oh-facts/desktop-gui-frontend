	...
	for(i32 i = 0; i < list->num; i ++)
	{
		...
		switch(pass->kind)
		{
			...
			case R_PASS_KIND_UI:
			{
				...
				{
					void *ssbo_data = glMapNamedBufferRange(r_opengl_state.inst_buffer[R_OPENGL_INST_BUFFER_UI], 
					0, sizeof(m4f) + batch->count * sizeof(R_Rect), GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
